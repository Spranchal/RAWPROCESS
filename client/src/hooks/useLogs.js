...
export const useAcceptSolution = () => {
  ...
  return useMutation({
    mutationFn: async ({ logId, commentId }) => {
      const res = await fetch(`${API_BASE}/feed/${logId}/solution/${commentId}/accept`, {
        method: 'PUT',
        headers: getHeaders(),
      });
      if (!res.ok) throw new Error('Failed to accept solution');
      return res.json();
    },
    onMutate: async ({ logId, commentId }) => {
      await queryClient.cancelQueries({ queryKey: ['logs'] });
      const previousLogs = queryClient.getQueryData(['logs']);
          ...
          logs: old.logs.map((log) => {
            if (log.id === logId) {
              const updatedComments = (log.comments || []).map(c => 
                c.id === commentId ? { ...c, accepted: 1 } : c
              );
              return { ...log, status: 'resolved', comments: updatedComments };
            }
            return log;
          }),
        };
